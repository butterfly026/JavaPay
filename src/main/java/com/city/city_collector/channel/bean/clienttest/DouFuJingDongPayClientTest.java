 ...
 * 对接上游示例
 */
public class DouFuJingDongPayClientTest extends ClientTest {

    //1. 商户资料
    public DouFuJingDongPayClientTest() {
        /**
         * 商户号
         */
        merchantNo = "8Jpn5osoq9Fuwnkq0Ilo";
        /**
         * 商户秘钥
         */
        merchantMy = "qNROEyV1amrV6uMCHMjJoGFnT";
        /**
         * 下单url
         */
        payUrl = "http://72.167.42.226/api/returndata.php";
        //???
        cashUrl = "";
        /**
         * 通道参数: 上游多通道使用
...
//            params.put("CategoryCode", "138");
        ...
        TreeMap<String, String> payParam = new TreeMap<>();
        payParam.put("out_logno", sn);
        String amountValue = amount.setScale(2, BigDecimal.ROUND_HALF_UP).toString();
        payParam.put("money", amountValue);
        String notifyUrl = ApplicationData.getInstance().getConfig().getNotifyurl() + Constants.ASYNC_URL + "/" + id;
        payParam.put("notifyurl", notifyUrl);

        Iterator<Map.Entry<String, String>> it = payParam.entrySet().iterator();
...
//        System.out.println(sign);
        ...
        params.put("body", new Gson().toJson(payParam));
...
//            SUCCESS
            ...
            if (!"200".equals(params.get("code").toString())) {
                return null;
            }
...
//            Iterator<Map.Entry<String, Object>> it = signParams.entrySet().iterator();
            ...
            sbuf.append(params.get("out_logno"));
            sbuf.append(params.get("money"));
            sbuf.append(params.get("paytime"));
            sbuf.append(merchantMy);

            String anObject = MD5Util.MD5Encode(sbuf.toString(), "UTF-8").toLowerCase();
            if (sign.equals(anObject)) {
                NotifyInfo ni = new NotifyInfo();
                ni.setSn((String) params.get("out_logno"));
                ni.setClientSn("");
