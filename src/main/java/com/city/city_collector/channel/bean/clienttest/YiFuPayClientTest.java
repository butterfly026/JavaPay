 ...
 * 对接上游示例
 */
public class YiFuPayClientTest extends ClientTest {

    //1. 商户资料
    public YiFuPayClientTest() {
        /**
         * 商户号
         */
        merchantNo = "7a58017fe1fa4b278cc26e3c733b24c0" ;
        /**
         * 商户秘钥
         */
        merchantMy = "86e392f50a6c4f9496a3494fe56f4371" ;
        /**
         * 下单url
         */
        payUrl = "http://merchant.vinda888.com/api/merchant/order" ;

//        47.75.126.239,47.75.126.25,47.75.125.13
        ...
        channelParams.put("payment_code","100");


//        移动微信扫码	100	8.00% [50,100,200,300,500]
    }
...
//            params.put("CategoryCode", "138");
        ...
        params.put("app_id", merchantNo);
        params.put("out_trade_id", sn);
        ...
        //1. 接接key
        sbuf.append("app_secret=" + merchantMy);
        //2. 最后key直接拼接
...
//        System.out.println(sign);
    ...
    public OrderInfo dealOrderData(String data, Long clientId, String clientNo, String clientName,
        ...
        if (!"ok".equals(jobj.get("state").getAsString())) {
            return null;
        }
    ...
    public NotifyInfo notifyData(Map<String, Object> params, String merchantNo, String merchantMy,
        ...
        //8. 上游回调通知
        try {
            String sign = (String) params.get("md5_sign");
            if (StringUtils.isBlank(sign)) {
                return null;
            }

            if(!"9".equals(params.get("order_status").toString())){
                return null;
            }

            Map<String, Object> signParams = new TreeMap<String, Object>();
            signParams.putAll(params);
            signParams.remove("md5_sign");
            signParams.remove("paid_time");
            ...
            if (sign.equals(anObject)) {
                ...
                ni.setSn((String) params.get("out_trade_id"));
                ni.setClientSn("");
