 ...
 * 对接上游示例
 */
public class YongChengPayClientTest extends ClientTest {

    public YongChengPayClientTest() {
        /**
         * 商户号
         */
        merchantNo = "";
         ...
         * 下单url
         */
        payUrl = "http://pay.yanmitang.com/Pay";
        //???
        cashUrl = "";
        /**
         * 通道参数: 上游多通道使用
    ...
    public OrderPreInfo createReqParams(Long id, String merchantNo, String merchantMy,
        ...
        String value = amount.setScale(2, BigDecimal.ROUND_HALF_UP).toString();
        params.put("fxfee", value);

        if (channelParams != null) {
            params.putAll(channelParams);
        }
        params.put("fxip", "127.0.0.1");

        params.put("fxid", merchantNo);
        params.put("fxddh", sn);
        params.put("fxdesc", "googleproduce");

        String notifyUrl = ApplicationData.getInstance().getConfig().getNotifyurl() + Constants.ASYNC_URL + "/" + id;
        params.put("fxnotifyurl", notifyUrl);

        params.put("fxbackurl", ApplicationData.getInstance().getConfig().getNotifyurl() + Constants.ASYNC_URL + "/" + id);


        String signStr = merchantNo + sn + value + notifyUrl + merchantMy;

//        System.out.println(signStr);
        String sign = MD5Util.MD5Encode(signStr, "UTF-8");
//        System.out.println(sign);
        params.put("fxsign", sign);
    ...
    public NotifyInfo notifyData(Map<String, Object> params, String merchantNo, String merchantMy,
        ...
        try {
//            success
            if (!"1".equalsIgnoreCase(params.get("fxstatus").toString())) {
                return null;
            }

            String sign = (String) params.get("fxsign");
            if (StringUtils.isBlank(sign)) {
                return null;
            }

            String signStr = String.valueOf(params.get("fxstatus")) + String.valueOf(params.get("fxid")) + String.valueOf(params.get("fxddh")) + String.valueOf(params.get("fxfee")) + merchantMy;
            String anObject = MD5Util.MD5Encode(signStr, "UTF-8");
            if (sign.equals(anObject)) {
                NotifyInfo ni = new NotifyInfo();
                ni.setSn((String) params.get("fxddh"));
                ni.setClientSn("");
...
//        System.out.println(sign);
        ...
        apiInfo.setSignStr(sign.toString());
