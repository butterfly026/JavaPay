...
<div class="admin-main">
    <form id="editForm" class="layui-form">
        ...
        <div style="color:red;width:90%;padding-left:20px;margin-bottom:10px;">【商户提现信息】<br/>账号:<span
                style="color:green;">${cash.bankAccno }</span>,持卡人:<span
                style="color:green;">${cash.bankAccname }</span>,银行:<span style="color:green;">${cash.bankName}</span>,IFSC:<span
                style="color:green;">${cash.bankIfsc}</span>,国家:<span style="color:green;">${cash.bankNation}</span>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <select name="payType" id="payType" lay-verify="required" lay-search lay-filter="up_type">
                    <option value="0"
                            <c:if test="${cash.payType==0}">selected</c:if> >人工代付
                    </option>

                    <option value="1"
                            <c:if test="${cash.payType==1}">selected</c:if> >API代付
                    </option>
        ...
        <div class="layui-form-item" id="rg_client"
             <c:if test="${cash.payType==1}">style="display:none;"</c:if> >
            ...
            <div class="layui-input-block">
                <select name="clientId" id="clientId" lay-verify="required" lay-search>
                    <c:forEach items="${clientList }" var="c">
                        <option value="${c.id}"
                                <c:if test="${cash.clientId==c.id}">selected</c:if> >${c.username}-${c.name}[余额${c.money} ${sysconfig.currency },冻结${c.frozenMoney } ${sysconfig.currency },可用${c.amount } ${sysconfig.currency }]
                        </option>
                    </c:forEach>
                </select>
            </div>
        ...
        <div class="layui-form-item" id="rg_client1"
             <c:if test="${cash.payType==0}">style="display:none;"</c:if> >
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="money" id="money" lay-verify="required|number" lay-verType="tips"
                       placeholder="提现金额" autocomplete="off" class="layui-input" value="${cash.clientMoney}"/>
            </div>
        </div>
        <!--
    ...
    $(function () {
        ...
        layui.use(['form', 'layer'], function () {
            ...
            form.on('submit(edit)', function (data) {
                ...
                layer.confirm('确定要更换代付上游？如果是API代付，请确认上游系统的代付已驳回。', {
                    icon: 3,//询问图标
                    btn: ['确定', '取消'] //按钮
                }, function (index) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: "merchantChangeClientCashSubmit",
