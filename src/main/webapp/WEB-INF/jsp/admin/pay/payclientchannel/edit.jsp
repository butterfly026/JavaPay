...
<div class="admin-main">
    <form id="editForm" class="layui-form">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="maxMoney" id="maxMoney" lay-verify="required|number" lay-verType="tips"
                       ...
                       value="${result==null?0:result.maxMoney}"/>
            </div>
        </div>
                ...
                <select name="keyname" lay-verify="required">
                    <c:forEach items="${modelList }" var="c">
                        <option value="${c.keyname}"
                                <c:if test="${result.keyname eq c.keyname}">selected</c:if> >${c.name} ${c.status==1?'(启用)':'(禁用)' }</option>
                    </c:forEach>
                </select>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <textarea name="params" id="params" placeholder="通道参数"
                          class="layui-textarea">${result.params}</textarea>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <textarea name="descript" id="descript" placeholder="通道描述"
                          class="layui-textarea">${result.descript}</textarea>
            </div>
        </div>

        <div class="layui-form-item">
            <label class="layui-form-label">优先平台</label>
            <div class="layui-input-block">
                <select name="primaryPlatform" lay-verify="required">
                    <option value="0" <c:if test='${result.primaryPlatform==0}'>selected</c:if>>通用</option>
                    <option value="1" <c:if test='${result.primaryPlatform==1}'>selected</c:if>>苹果</option>
                    <option value="2" <c:if test='${result.primaryPlatform==2}'>selected</c:if>>安卓</option>
                    <option value="3" <c:if test='${result.primaryPlatform==3}'>selected</c:if>>其它</option>
                </select>
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">通道尝试次数(1~5)</label>
            <div class="layui-input-block">
                <input type="number" name="retryNumber" id="retryNumber" min="1" max="5" lay-verify="required|number" lay-verType="tips"
                       placeholder="通道尝试次数" autocomplete="off" class="layui-input"
                       value="${result==null?1:result.retryNumber}"/>
            </div>
        </div>
