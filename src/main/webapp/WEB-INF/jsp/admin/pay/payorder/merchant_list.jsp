...
<div class="admin-main">
    ...
    <div class="admin-btns">
        <adminPermission:hasPermission permission="admin:payorder:merchantExport">
    ...
    <script type="text/html" id="tpl_opera">
		...
		<adminPermission:hasPermission permission="admin:payorder:merchantList">
			{layIcon:'layui-icon layui-icon-form',txt: '详情',event:'detail'},
		</adminPermission:hasPermission>

		{{# if(d.order_status==1){ }}
		<adminPermission:hasPermission permission="admin:payorder:merchantXy">
			{layIcon:'layui-icon layui-icon-speaker',txt: '通知下游',event:'xy'},
		</adminPermission:hasPermission>
		{{# } }}
	]}">
            <span>操作</span>
...
<script type="text/javascript">
    ...
    var exportObj = [
        [ //表头
            ...
            {field: 'ratio', title: '费率', width: 15, rowspan: 1, colspan: 1},
            {field: 'commission', title: '手续费(元)', width: 20, rowspan: 1, colspan: 1},
            {field: 'realMoney', title: '实际到账(元)', width: 20, rowspan: 1, colspan: 1}
        ]
    ];
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.code == 200) {
                    var $r = data.result;
                    if ($r != null) {
                        $("#totalInfo").html("当前数据中,交易成功金额<span style='color:green'>" + ($r.sucMoney ? $r.sucMoney : '0') + "</span>元,手续费<span style='color:green'>" + ($r.commission ? $r.commission : '0') + "</span>元,实际到账<span style='color:green'>" + ($r.realMoney ? $r.realMoney : '0') + "</span>元");
                    } else {
