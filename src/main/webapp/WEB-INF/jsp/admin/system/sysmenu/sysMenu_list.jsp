...
<div class="admin-main">
    ...
    <div class="admin-btns">
        <adminPermission:hasPermission permission="admin:sysmenu:add">
...
<script>
    ...
    function del(id) {
        ...
        layer.confirm('您真的要删除这条菜单吗？如果包含子菜单，子菜单会一起删除。', {
            icon: 3,//询问图标
            btn: ['确定', '取消'] //按钮
        }, function (index) {
    ...
    function createTable(mtrees, nprev) {
        ...
        for (var i = 0; i < mtrees.length; i++) {
            ...
            if (mtrees[i].data.type && mtrees[i].data.type == 1) {//按钮显示黑色
                $str += '&nbsp;<span style="color:#000000;">' + nullStr(mtrees[i].data.name) + '</span></td>';
            } else {
                $str += '&nbsp;<span style="color:#6A6AFF;">' + nullStr(mtrees[i].data.name) + '</span></td>';
            }

            $str += '<td>' + nullStr(mtrees[i].data.url) + '</td>';

            $str += '<td>' + ((mtrees[i].data.image == null || mtrees[i].data.image == "") ? '' : ('<img src="' + mtrees[i].data.image + '" width="16" height="16">')) + '</td>';

            $str += '<td>' + nullStr(mtrees[i].data.order) + '</td>';
            if (mtrees[i].data.status && mtrees[i].data.status == 1) {
                $str += '<td><span class="layui-badge layui-bg-blue">启用</span></td>';
            } else {
                $str += '<td><span class="layui-badge">禁用</span></td>';
            }
            if (mtrees[i].data.type && mtrees[i].data.type == 1) {
                $str += '<td><span class="layui-badge layui-bg-black">按钮</span></td>';
            } else {
                $str += '<td><span class="layui-badge layui-bg-blue">菜单</span></td>';
            }
            $str += '<td>' + nullStr(mtrees[i].data.permission) + '</td>';

            $str += '<td>';
            <adminPermission:hasPermission permission="admin:sysmenu:edit">
            $str += '<a class="layui-btn layui-btn-xs layui-btn-normal" href="javascript:void(0);" onclick="edit(\'' + mtrees[i].data.id + '\')">编辑</a>';
            </adminPermission:hasPermission>
            <adminPermission:hasPermission permission="admin:sysmenu:delete">
